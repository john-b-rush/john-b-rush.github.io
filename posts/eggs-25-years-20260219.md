...
Why can't we just use you? I'd been testing five approaches to a problem that Claude was already solving, in passing, every time I pasted a receipt into our chat. So we did. Claude Sonnet's vision on all 2,163 receipts. Sometimes the answer is embarrassingly obvious once you stop engineering around it.
...
So I told Codex "we have unlimited tokens, it's no big deal," and we pivoted to sending everything to a vision model. From that one sentence, Codex came back with a parallel worker architecture — sharding, health management, checkpointing, retry logic. The whole thing. When Claude's CLI login failed mid-run, Codex auto-disabled Claude and continued solo. I didn't ask it to do that. I didn't know it had happened until I read the logs.
...
I spent a surprising amount of time saying no. "I don't know why we need to hash the store id, we can just use it." The agents built four separate QA tools for me. I had to use all four. They kept proposing new abstractions for problems that didn't need abstracting. Good builders. Terrible editors.
...
But it still took $600, 14 days, and me catching bugs the agents missed. Runs crashed at 3 AM. A $9.99 egg purchase was invisible until I opened the HTML myself. We're close — and close is what matters, because the pipeline exists now. Five years from now I'll run it again. Same scans, better models, probably ten minutes instead of fourteen days. That's the thing about keeping data: you only have to wait once.
