...
One API call with `text="receipt"`. 0.92-0.98 confidence on every boundary. Four seconds per scan. 507 receipts from 760 multi-receipt pages. Seven approaches in hours; SAM3 in an afternoon.
...
10,949 receipts processed. The system that was projected to take all night finished before I went to bed.
...
The fix: use those hand-labeled edge cases as few-shot examples in an LLM classifier. Twenty examples of what "eggs" looks like on a garbled thermal print from 2003. Batch 10 receipts per call. Eight parallel workers. Two hours. 14,999 receipts classified.
